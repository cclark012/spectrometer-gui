...
def _describe_native_argument(argument: Any) -> str:
    """Return a compact, non-dereferencing description for debug breadcrumbs."""

    if isinstance(argument, ctypes.Array):
        try:
            address = ctypes.addressof(argument)
            address_text = f"0x{address:X}"
        except (TypeError, ValueError):
            address_text = "unknown"
        return (
            f"{type(argument).__name__}(length={len(argument)}, "
            f"bytes={ctypes.sizeof(argument)}, address={address_text})"
        )

    if hasattr(argument, "value"):
        try:
            return f"{type(argument).__name__}(value={argument.value!r})"
        except Exception:
            pass

    rendered = repr(argument)
    if len(rendered) > 200:
...
def _probe_spectrograph(root: Path, dll_path: Path) -> dict[str, Any]:
    ...
    def record_call(
        ...
        *args: Any,
        ...
        """Record a native call with flushed before/after crash breadcrumbs."""

        _debug(
            "CALL",
            function_name,
            f"available={function is not None}",
            f"argtypes={getattr(function, 'argtypes', None)}",
            "args=",
            [_describe_native_argument(argument) for argument in args],
        )
        result = recorder.record(
            function_name,
            function,
        ...
        _debug("RETURN", function_name, result)
        return result
    ...
    init_result = record_call(initialize_name, initialize, os.fsencode(str(root)))
    ...
    try:
        ...
        for device_index in range(max(0, int(count.value))):
            ...
            if prefix == "ATSpectrograph":
                # The ATSpectrograph ABI adds maxSerialStrLen after serial.
                # The legacy Shamrock ABI does not include this parameter.
                serial_argtypes.append(ctypes.c_int)
            function_name, function = optional(
                "GetSerialNumber", serial_argtypes
            )
            ...
            _debug(
                "GetSerialNumber ABI:",
                prefix,
                f"serial_buffer_bytes={serial_buffer_bytes}",
                f"argtypes={serial_argtypes}",
                function,
            )

            queries["serial_number"] = record_call(
                function_name,
                function,
                *serial_args,
            ...
            grating_count_result = record_call(
                function_name,
                function,
            ...
            queries["current_grating"] = record_call(
                function_name,
                function,
                ctypes.c_int(device_index),
            ...
            queries["wavelength_nm"] = record_call(
                function_name,
                function,
                ctypes.c_int(device_index),
            ...
            queries["optical_parameters"] = record_call(
                function_name,
                function,
                ctypes.c_int(device_index),
            ...
            if prefix == "ATSpectrograph":
                # The ATSpectrograph ABI adds maxBlazeStrLen after Blaze.
                # The legacy Shamrock ABI does not include this parameter.
            ...
            get_info_name, get_info = optional(
                "GetGratingInfo", grating_info_argtypes
            )
            _debug(
                "GetGratingInfo ABI:",
                prefix,
                f"blaze_buffer_bytes={blaze_buffer_bytes}",
                f"argtypes={grating_info_argtypes}",
                get_info,
            )
            ...
            for grating_index in range(1, max(0, int(grating_count.value)) + 1):
                ...
                _debug(
                    f"Preparing grating {grating_index}/{grating_count.value}",
                    f"using {prefix} ABI",
                )
                info_result = record_call(
                    get_info_name,
                    get_info,
                    *grating_info_args,
                ...
                _debug(
                    f"Grating {grating_index} wavelength limits result:",
                    limits_result,
                )
                gratings.append(
                    {
            ...
            shutter_present_result = record_call(
            ...
            if shutter_present_result.code == ATSPECTROGRAPH_SUCCESS and shutter_present.value:
                ...
                queries["shutter_state"] = record_call(
                    shutter_name,
                    get_shutter,
                    ctypes.c_int(device_index),
            ...
            focus_present_result = record_call(
                focus_present_name,
                focus_present_fn,
                ctypes.c_int(device_index),
            ...
            if focus_present_result.code == ATSPECTROGRAPH_SUCCESS and focus_present.value:
                ...
                focus = ctypes.c_int()
                queries["focus_mirror_position"] = record_call(
                    focus_name,
                    get_focus,
                    ctypes.c_int(device_index),
                    ctypes.byref(focus),
            ...
            queries["pixel_width_um"] = record_call(
                pixel_width_name,
                get_pixel_width,
                ctypes.c_int(device_index),
            ...
            pixel_count_result = record_call(
                pixel_count_name,
                get_pixel_count,
                ctypes.c_int(device_index),
            ...
            if (
                ...
                calibration_result = record_call(
                    calibration_name,
                    get_calibration,
                    ctypes.c_int(device_index),
                    calibration,
        ...
        report["close"] = record_call(close_name, close)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        # On Windows, faulthandler can emit a Python stack for fatal native
        # exceptions such as access violations before the process terminates.
